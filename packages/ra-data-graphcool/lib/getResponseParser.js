    ...
    var result = Object.keys(data).reduce(function (acc, key) {
        ...
        if (linkedResource) {
            ...
            if (Array.isArray(linkedResourceData)) {
                return __assign(__assign({}, acc), (_b = {}, _b[field.name] = data[field.name].map(sanitizeResource(introspectionResults, linkedResource)), _b[field.name + "Ids"] = data[field.name].map(function (d) { return d.id; }), _b));
            }
